...
def main(d_obs, run, rep, b, b_prime, alpha, t0_val, sample_size_obs, classifier_cde, test_statistic,
    ...
    classifier_dict = {k: v for k, v in classifier_dict_complete.items() if k.lower() == classifier}
    ...
    out_cols = ['d_obs', 'test_statistic', 'b_prime', 'b', 'classifier', 'classifier_cde', 'run', 'rep', 'sample_size_obs',
                'cross_entropy_loss', 't0_true_val', 'coverage', 'coord_val', 'in_confint', 'true_entropy', 'or_loss_value',
                'monte_carlo_samples', 'benchmark', 'nuisance_parameters', 'guided_sim',
                'guided_sample', 'uniform_grid_sample_size']
    ...
    for clf_name, clf_model in sorted(classifier_dict.items(), key=lambda x: x[0]):
        ...
        pbar = tqdm(total=rep, desc='Generating samples and evaluate odds, n=%s, b=%s' % (sample_size_obs, b))
        for nn in range(rep):
        ...
        if test_statistic == 'acore':
            ...
            for kk, theta_0 in enumerate(theta_mat):
                stats_mat[kk] = compute_bayesfactor_single_t0(
    ...
    for clf_name, (tau_obs_val_list, cross_ent_loss_list, or_loss_value_list) in clf_odds_fitted.items():
        for clf_name_qr, cutoff_val in clf_cde_fitted[clf_name].items():
            ...
            for tt, tau_obs_val in enumerate(tau_obs_val_list):
                ...
                for hh, in_confint_val in enumerate(in_confint):
                    out_val.append([
                        d_obs, test_statistic, b_prime, b, clf_name, clf_name_qr, run, tt, sample_size_obs,
                        cross_ent_loss_list[tt], t0_val, coverage, t0_grid[hh, 0],
                        in_confint_val, entropy_est, or_loss_value_list[tt], monte_carlo_samples, benchmark,
                        int(nuisance_parameters), int(guided_sim), guided_sample,
                        uniform_grid_sample_size
                    ])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--t0_val', action="store", type=float, default=0.0,
    ...
    parser.add_argument('--classifier', action="store", type=str, default='mlp',
                        help='Classifier for odds estimation')
