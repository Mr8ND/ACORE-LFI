...
class ToyGMMMultiDLoader:

    def __init__(self, d_obs, mean_instrumental=0, std_instrumental=10, low_int=0, high_int=10, true_param=5.0,
                 out_dir='toy_gmm_multid/', sigma_mixture=(1.0, 1.0), mixing_param=0.5,
                 marginal=False, size_marginal=1000):

        self.mean_instrumental = np.repeat(mean_instrumental, d_obs)
        self.std_instrumental = np.diag(np.repeat(std_instrumental, d_obs))
    ...
    def _gmm_manual_sampling(self, sample_size, mu_param):
        cluster = np.repeat(np.random.binomial(n=1, p=self.mixing_param, size=sample_size), self.d_obs)
        means = np.take([-1 * mu_param, mu_param], cluster)
        sigmas = np.take([self.sigma_mixture[0], self.sigma_mixture[1]], cluster)
        return np.random.normal(loc=means, scale=sigmas, size=sample_size * self.d_obs).reshape(sample_size, self.d_obs)
