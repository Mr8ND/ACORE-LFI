...
class ToyMVNMultiDLoader:
    ...
    def __init__(self, d_obs=2, mean_instrumental=0, std_instrumental=4, low_int=0, high_int=10,
                 true_param=[5,5], true_std=1,
        ...
        assert(len(true_param) == d_obs)
        self.true_param = np.array(true_param)
        self.true_cov = true_std * np.eye(d_obs)
    ...
    def sample_param_values(self, sample_size):
        if self.prior_type == 'uniform':
            unique_thetas = np.random.uniform(low=self.low_int, high=self.high_int, size=sample_size * self.d_obs)
            return unique_thetas.reshape(sample_size, self.d_obs)
        elif self.prior_type == 'normal':
            unique_thetas = np.random.normal(loc=self.mean_instrumental, scale=self.std_instrumental, size=sample_size * self.d_obs)
            return unique_thetas.reshape(sample_size, self.d_obs)
        else:
    ...
    def generate_sample(self, sample_size, p=0.5, **kwargs):
        ...
        sample = np.apply_along_axis(arr=concat_mat, axis=1,
                                     func1d=lambda row: self.sample_sim(sample_size=1, true_param=row[:self.d_obs]) if row[self.d_obs]
    ...
    def sample_msnh_algo5(self, b_prime, sample_size):
        ...
        sample_mat = np.apply_along_axis(arr=theta_mat, axis=1,
                                         func1d=lambda row: self.sample_sim(sample_size=sample_size, true_param=row[:self.d_obs]))
    ...
    def compute_exact_prob(self, theta_vec, x_vec, p=0.5):
        ...
        f_val = np.array([multivariate_normal.pdf(
            x=x, mean=theta_vec[ii], cov=self.true_cov) for ii, x in enumerate(x_vec)]).reshape(-1, )
