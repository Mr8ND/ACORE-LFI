...
class ToyGammaLoader:
    ...
    def compute_marginal_reference(self, size_marginal):
        ...
        marginal_sample = np.random.gamma(shape=theta_vec_marg, scale=self.scale, size=size_marginal)
    ...
    def compute_exact_or(self, t0, t1, x_obs):
        f0_val = gamma.pdf(x=x_obs.reshape(-1, ), a=t0, scale=self.scale)
        f1_val = gamma.pdf(x=x_obs.reshape(-1, ), a=t1, scale=self.scale)

        return f0_val / f1_val

    def compute_exact_prob(self, theta_vec, x_vec, p=0.5):
        f_val = gamma.pdf(x=x_vec.reshape(-1, ), a=theta_vec.reshape(-1, ), scale=self.scale)
    ...
    def compute_exact_likelihood(self, x_obs, true_param):
        return gamma.pdf(x=x_obs.reshape(-1, ), a=true_param, scale=self.scale)

    def compute_exact_lr_simplevsimple(self, x_obs, t0, t1):
        '''
        Compute the exact likelihood ratios for normal case.
        '''
        ll_t0 = gamma.pdf(x=x_obs.reshape(-1, ), a=t0, scale=self.scale)
        ll_t1 = gamma.pdf(x=x_obs.reshape(-1, ), a=t1, scale=self.scale)
        return np.sum(np.log(ll_t0) - np.log(ll_t1))

    @staticmethod
    def compute_mle(x_obs):
        raise NotImplementedError("Still having to implement it.")
    ...
    def compute_exact_lr_simplevcomp(self, x_obs, t0, mle):
        ...
        '''
        ll_t0 = gamma.pdf(x=x_obs.reshape(-1, ), a=t0, scale=self.scale)
        ll_mle = gamma.pdf(x=x_obs.reshape(-1, ), a=mle, scale=self.scale)
